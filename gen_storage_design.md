...
The CouchDB and Couchstore storage system uses a tail append design, where all updates to a database result in appending new or updated information to the storage file. Even deletes would grow the storage files until compaction.
...
An improvement of this new design over CouchDB and previous Couchstore designs are the optimization for Zipfian/Zeta distributions of document update frequencies.
...
## Components of a Couchbase 2.0 Storage File
...
The tail-append design means additional seeks are unnecessary to write the data (excluding file system metadata), which minimizes write amplification in SSDs and disk seeks on HDDs.
...
The tail append design ensures data integrity of committed data regardless of orderly shutdown or crash. Should a write fail to complete because of any kind of crash, even power loss, previous data written and indexes written to the file are still intact and untouched. And incomplete writes and index updates simply appear as garbage data at the end of the file, and are ignored on database open. This is due to the pure tail append design, which means special crash recovery code, such as consistency checks or fixups, are never needed.
...
The btree's are immutable and all updates are Copy-On-Write. This means for an update in the btree, no existing btree nodes (root, inner or leaf) are  modified, but rather updated in memory and copied to the end of the file. Therefor to update a leaf node, it's rewritten and it's parent node must also be rewritten to point to the new node. And it's parent, and so on, until finally the root is recopied. Only the nodes from the modified leaf to the root are rewritten, all other nodes remain in their original locations.
...
Disk space inefficiency can occur when a compaction to a partition occurs and others are still reading a snapshot of the file, the old file cannot be truly deleted and space recovered by the OS until all threads and processes reading the file have finished and close the file. Once that occurs the OS will fully recover all space used by the file. Very slow readers can potentially keep these files open for a long time.

The efficiency of snapshotting is particularly important when performing range scans of by\_seq index for UPR, which requires UPR clients (backup utilities, indexers, replicators, etc) are able to get a consistent and complete snapshot to ensure fast recovery points in failover scenarios.

## Compaction

Compaction is the process of recovering wasted space in a storage file and arranging all the live data and btrees to be optimized for size and arrangement. It copies all data and btree indexes from the old file to a new file. It does this without without stopping or pausing persistence and fetches, but can result in duplicate IO for writes that occurred during the compaction.
...
The cost of compaction per document updated is 0((N*log(N))/M) where N is the total number of documents in the file and M is total number of unique documents updated between compactions.
...
The best case for compaction is when there is a large number of single updates across all documents, so as M becomes large and approaches N, the compaction cost become O(log(N)) per unique document update.
...
# Generational Storage System Architecture
...
On startup, the WAL file or files are read from beginning to end into memory, with subsequent updates to the same document replacing in the in-memory representation of the earlier mutation. These newly read documents are still marked un-ejectable from RAM until they are written and committed to partition specific storage. Every file is read until the end or an item in it is found to be corrupt (incorrect length or CRC32).

The advantage of the WAL is to reduce the latency of a durable commit, as it only has to wait on a single fsync of a sequential write for all partitions, instead of an fsync per partition, which can increase latency dramatically as each partition must be written to and fsynced before all pending writes can be completed.
...
A large RAM cache will improve the performance for a working set that can fit into it entirely, but if not large enough and if there is a strong correlation between how often a document is updated and how often it's read, GS will also provide cheaper disk based accesses, finding documents in smaller, hotter requires fewer btree reads to reach a document.

However if there is little connection between updates and reads, generational storage will offer slightly less fetch efficiency over a single storage file (due to statistically more btree nodes reads necessary), or theoretically a storage scheme that optimizes disk layout for an evenly distributed read frequency (such as traditional balanced btrees).
...
CompactionVersion is a monotonically increasing integer C that is the same as the number of times the generation has been compacted in-place. For a small window of time as compaction completes, there will be 2 files, the older file being C and the new file being C+1. Once version C+1 is in place, the older file C will be deleted.
...
* Higher false positive ratios of the bloom filters. When a generation is newly created, the bloom filters will have their lowest average false positive rate. As new documents are added to the generation, the false positive rate will increase.
...
Another advantage of generational storage over single file storage is to reduce the rate of fragmentation of documents and btree nodes.

This is because the rate at which documents and btree nodes become fragmented are highly affected by the batch size of documents (larger batches are better). The best case for batching is experienced with compaction, where all documents written to a new a file in a single batch, resulting in documents that are sequentially packed together, and btree's nodes exhibiting ideal locality for random lookups and range scans.
...
All mutations to partition GS happen first in the WAL (shared by all partitions), then at the smallest generation of the specific partition. 
...
A background process then writes the updated document(s) and metadata into the smallest generation file. When the generation exceeds its live data threshold, the coldest documents are copied to the next colder generation, the hotter documents are written to a new file of the same generation level.

### by\_seq Tombstone

If the mutation is an edit or deletion, the old by\_seq entry the edit is overwriting must be deleted and a new one written. If the previous by\_seq entry is still in memory and WAL only and uncopied to generational storage, nothing needs to be done. If the previous generation is in GS or it's unknown if it's in generational storage (due to concurrent migration of the mutation from WAL to GS), then write a deletion "tombstone" for the by\_seq entry to be copied into generational storage.
...
We no longer have to align the storage file header to any boundary, so we can pack headers contiguous with previous data with no gaps in between. This will save an average of 1/2 the block size per commit (4k by default, so 2k on average . For larger generations, this will have small benefit as commits are infrequent and large.
...
With the larger, colder generation files, the bloom filter can be very large. Approximately 9 bits per document stored for a bloom filter with 1% false positive rate. However, the cost of rewriting the whole bloom filter will generally be offset by the cost of the bulk writes to the file, since the cost each update will be due to relatively large amount of documents migrating from hotter storage to colder. The colder the file, the larger the bulk update and the larger the bloom filter, so the amortized cost of rewriting the bloom filter per doc migrated should remain consistent and relatively small at all generation levels.

### False Positive Degradation and Regeneration

As more unique documents are added to the file, the false positive rate of the bloom filter will increase. But when the file is compacted or copied to the next generation, the entire bloom filter for the generation will be regenerated from scratch and brought up to maximal space vs false positive efficiency.

## MVCC and Range Scans

When traversing the partition by\_seq all files will be scanned simultaneously with the btree index in each file will be walked simultaneously, and a merge sort will occur, with the hottest/newest entry used. The in-memory WAL data and snapshot-able in memory by\_seq entry (as specified in the UPR design specification) is also walked.

If duplicate entries are found, a common and normal occurrence, the entry from the hottest generation file will be used, with entries from colder generations discarded or amended. If the hottest entry is a "metadata deletion" entry, all metadata for the same sequence will be skipped. If the hottest entries are "metadata update" entries, the updates are applied to the hottest concrete entry in order of coldest to hottest and returned to the caller.
